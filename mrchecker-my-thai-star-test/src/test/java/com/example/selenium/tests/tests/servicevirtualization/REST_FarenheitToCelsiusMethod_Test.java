package com.example.selenium.tests.tests.servicevirtualization;
...
public class REST_FarenheitToCelsiusMethod_Test extends BaseTest {
  ...
  public void testRestMock_UrlExists() {
    ...
    Response response = DriverManager.getDriverWebAPI().with().header("Content-Type", ContentType.JSON.toString()).log()
        .all().when().get(endpointBaseUri + "/some/thing").thenReturn();

    BFLogger.logInfo("#4 Validate response ");
    BFLogger.logDebug("/some/thing: " + response.jsonPath().prettyPrint());
  ...
  public void testRestMock_UrlDoesNotExist() throws Exception {
    ...
    BFLogger.logInfo("#3 Send request to generated stub");
    Response response = DriverManager.getDriverWebAPI().with().header("Content-Type", ContentType.JSON.toString()).log()
        .all().when().get(endpointBaseUri + "/some/thing/else").thenReturn();
  ...
  public void testRestMock_post() throws Exception {
    ...
    BFLogger.logInfo("#3 Send request to generated stub");
    Response response = DriverManager.getDriverWebAPI().with().header("Content-Type", ContentType.JSON.toString())
        .body("{\"FahrenheitToCelsius\":{\"Fahrenheit\":10}}").log().all().when().post(endpointBaseUri + "/some/thing")
        .thenReturn();
